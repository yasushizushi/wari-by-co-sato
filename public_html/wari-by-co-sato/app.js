...
async function loadGroup(code) {
    ...
    try {
        const data = await apiRequest(`api/groups.php?action=get_group&code=${encodeURIComponent(normalizedCode)}`);
        state.group = data.group;
        state.code = state.group.code;
...
document.getElementById('share-link').addEventListener('click', async () => {
    if (!state.group) return;
    const url = `${window.location.origin}${basePath}?code=${state.group.code}`;
